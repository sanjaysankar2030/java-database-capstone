...
public class DoctorService {
        ...
        private AppointmentRepository appointmentRepostitory;
        ...
        public List<String> getDoctorAvailability(Long doctorId, LocalDate date) {
                ...
                List<Appointment> appointments = appointmentRepostitory.findByDoctorIdAndAppointmentTimeBetween(
                                doctorId,
                                startOfDay, endOfDay);
                ...
                List<String> allSlots = Arrays.asList(
                                "09:00", "10:00", "11:00",
                                "14:00", "15:00", "16:00");
                List<String> filteredList = allSlots.stream()
        ...
        public int saveDoctor(Doctor newDoctor) {
                try {
                        Doctor existingDoctor = doctorRepository.findByEmail(newDoctor.getEmail());

                        if (existingDoctor != null) {
                                return -1; // conflict (email already exists)
                        }

                        doctorRepository.save(newDoctor); // ✅ actual save
                        return 1; // success
        ...
        public int updateDoctor(Doctor updatedDoctor) {
                try {
                        Doctor existingDoctor = doctorRepository.findByEmail(updatedDoctor.getEmail());
        ...
        public int deleteDoctor(long doctorId) {
                try {
                        ...
                        appointmentRepostitory.deleteAllByDoctorId(doctorId);
        ...
        public ResponseEntity<Map<String, String>> validateDoctor(Login login) {
                ...
                try {
                        Doctor existingDoctor = doctorRepository.findByEmail(login.getEmail());
                        if (existingDoctor == null) {
                                return ResponseEntity.status(HttpStatus.UNAUTHORIZED).body(response);
                        }
                        if (!existingDoctor.getPassword().equals(login.getPassword())) {
    ...
    response.put("doctors", filteredList);
